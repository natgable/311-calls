...
nyc_2010_file <- "~/Desktop/DCL/C01/nyc/nyc_2010.rds"
nyc_2011_file <- "~/Desktop/DCL/C01/nyc/nyc_2011.rds"
nyc_2012_file <- "~/Desktop/DCL/C01/nyc/nyc_2012.rds"
nyc_2013_file <- "~/Desktop/DCL/C01/nyc/nyc_2013.rds"
nyc_2014_file <- "~/Desktop/DCL/C01/nyc/nyc_2014.rds"
nyc_2015_file <- "~/Desktop/DCL/C01/nyc/nyc_2015.rds"
nyc_2016_file <- "~/Desktop/DCL/C01/nyc/nyc_2016.rds"
nyc_2017_file <- "~/Desktop/DCL/C01/nyc/nyc_2017.rds"
...
write_rds(x = nyc_2010, path = nyc_2010_file, compress = "gz")
write_rds(x = nyc_2011, path = nyc_2011_file, compress = "gz")
write_rds(x = nyc_2012, path = nyc_2012_file, compress = "gz")
write_rds(x = nyc_2013, path = nyc_2013_file, compress = "gz")
write_rds(x = nyc_2014, path = nyc_2014_file, compress = "gz")
write_rds(x = nyc_2015, path = nyc_2015_file, compress = "gz")
write_rds(x = nyc_2016, path = nyc_2016_file, compress = "gz")
write_rds(x = nyc_2017, path = nyc_2017_file, compress = "gz")
```

Now for some interactive mapping!

```{r}
mod_police_calls_2010 <-
  police_calls_2010 %>% 
...
labels_2017 <- 
  sprintf(
  "Average Monthly Rent: $%g<br/>Median Household Income: $%g<br/>Percent White: %g",
  round(nyc_2017$avg_rent), 
  round(nyc_2017$med_income),
  round(100 * nyc_2017$percent_white)
  ) %>% lapply(htmltools::HTML)
white_vals <- 
  colorNumeric(
    palette = "Blues",
    domain = nyc_2017$percent_white
  )
leaflet() %>%  
  addProviderTiles(providers$CartoDB.Positron) %>% 
  setView(-73.95, 40.78, 11) %>% 
  addPolygons(
    data = nyc_2017$geometry,
    weight = 1,
    fillOpacity = 1,
    color = white_vals(white_2017$percent_white),
    label = labels_2017,
    highlightOptions = highlightOptions(
      color = "red", 
      weight = 3,
  ...
  addLegend("bottomright", pal = white_vals, values = nyc_2017$percent_white,
    title = "Percentage White",
    labFormat = labelFormat(suffix = "%"),
    opacity = 1,
    na.label = NULL
