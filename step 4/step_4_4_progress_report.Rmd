...
calls_per_tract <-
  st_contains(nyc_2010$geometry, mod_police_calls_2010) %>% 
  as_tibble() %>% 
  group_by(row.id) %>% 
  transmute(
    num_calls = n()
...
labels_2010 <- 
  sprintf(
    "Average Monthly Rent: $%g<br/>Median Household Income: $%g<br/>Percent White: %g<br/>Number of noise complaints: %g",
...
leaflet() %>%  
  ...
  addPolygons(
    ...
    weight = 1,
    fillOpacity = 1,
    color = calls_vals(nyc_2010$num_calls),
    label = labels_2010,
  ...
  addLegend("bottomright", pal = calls_vals, values = nyc_2010$num_calls,
    title = "Number of calls per census tract",
    opacity = 1,
