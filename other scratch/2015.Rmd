...
file_nyc_2015 <- "~/Desktop/DCL/C01/nyc/nyc_2015.rds"
nyc_2015 <- read_rds(file_nyc_2015)
...
call_geometries <-
  ...
  st_intersection(nyc_2015$geometry)

calls_per_tract <-
  st_contains(nyc_2015$geometry, call_geometries) %>% 
  as_tibble() %>% 
  group_by(row.id) %>% 
  transmute(
    num_calls = n()
...
write_rds(
  x = nyc_2015_with_calls, 
  path = nyc_2015_with_calls_file, 
  compress = "gz"
)
```