...
file_nyc_2017 <- "~/Desktop/DCL/C01/nyc/nyc_2017.rds"
nyc_2017 <- read_rds(file_nyc_2017)
...
call_geometries <-
  ...
  st_intersection(nyc_2017$geometry)

calls_per_tract <-
  st_contains(nyc_2017$geometry, call_geometries) %>% 
  as_tibble() %>% 
  group_by(row.id) %>% 
  transmute(
    num_calls = n()
...
write_rds(
  x = nyc_2017_with_calls, 
  path = nyc_2017_with_calls_file, 
  compress = "gz"
)
```