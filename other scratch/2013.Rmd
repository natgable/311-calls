...
file_nyc_2013 <- "~/Desktop/DCL/C01/nyc/nyc_2013.rds"
nyc_2013 <- read_rds(file_nyc_2013)
...
call_geometries <-
  ...
  st_intersection(nyc_2013$geometry)

calls_per_tract <-
  st_contains(nyc_2013$geometry, call_geometries) %>% 
  as_tibble() %>% 
  group_by(row.id) %>% 
  transmute(
    num_calls = n()
...
write_rds(
  x = nyc_2013_with_calls, 
  path = nyc_2013_with_calls_file, 
  compress = "gz"
)
```
