...
file_nyc_2014 <- "~/Desktop/DCL/C01/nyc/nyc_2014.rds"
nyc_2014 <- read_rds(file_nyc_2014)
...
call_geometries <-
  ...
  st_intersection(nyc_2014$geometry)

calls_per_tract <-
  st_contains(nyc_2014$geometry, call_geometries) %>% 
  as_tibble() %>% 
  group_by(row.id) %>% 
  transmute(
    num_calls = n()
...
write_rds(
  x = nyc_2014_with_calls, 
  path = nyc_2014_with_calls_file, 
  compress = "gz"
)
```